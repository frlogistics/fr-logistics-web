...
(function () {
  ...
  // -----------------------------------------------------------
  // LIAM — chat propio (canal 'web')
  // El boton mascot abre el widget fr-chat-widget.js, que habla con
  // /.netlify/functions/web-chat y de ahi al mismo agente LIAM que
  // atiende WhatsApp. Todo cae en wa_messages y en el inbox del portal.
  // Fallback a WhatsApp solo si el widget no cargo.
  // Reemplazo de la integracion con Tidio (2026-08-24).
  // -----------------------------------------------------------
  const liamBtn = document.getElementById('liamChatBtn');
  if (liamBtn) {
    const WHATSAPP_FALLBACK = 'https://api.whatsapp.com/send?phone=13052403172&text=Hola%20LIAM%2C%20quiero%20hablar%20con%20FR-Logistics';

    liamBtn.addEventListener('click', (e) => {
      e.preventDefault();
      if (window.FRChat && typeof window.FRChat.open === 'function') {
        window.FRChat.open();
        return;
      }
